...
class VectorIterator[+A](startIndex: Int, final override private[Immutable] val endIndex: Int)
  ...
  private[Immutable] def remaining: Int = spire.math.max(endIndex - blockIndex + lo, 0)
  ...
  final private[Immutable] def gotoNextBlock(): Unit = {
    ...
    val oldBlockIndex: Int = blockIndex
    val newBlockIndex: Int = oldBlockIndex + endLo

    blockIndex = newBlockIndex
    lo = 0

    if (newBlockIndex < focusEnd) {
      val _focusStart: Int = focusStart
      val newBlockIndexInFocus: Int = newBlockIndex - _focusStart
  ...
  /** Applies a function `f` to all elements of this $coll.
    ...
    *
    */
  def foreach[U](f: A => U): Unit = { while (this.hasNext) f(this.next()) }
  ...
  /** Advances this iterator past the first ''n'' elements, or the length of the iterator, whichever is smaller.
    ...
    */
  def drop(n: Int): VectorIterator[A] = {
    var j = 0
    while (j < n && this.hasNext) {
      this.next()
      j += 1
    } // TODO Make it a constant time operation and not a linear time one
    this
  }
  ...
  def indexWhere(p: A => Boolean, from: Int): Int = {
    var i: Int = 0
    //this.drop(from) TODO Implement using drop
  ...
  /** Tests if another iterator produces the same values as this one.
    ...
    */
  def sameElements(that: VectorIterator[_]): Boolean = {
